...
### 지금은 다 같은 모델
def get_coin_model(img_height, img_width, num_classes):
    inputs = Input(shape=(img_height, img_width, 3))

    x = Conv2D(filters=16, kernel_size=3, strides=2, activation='relu')(inputs)
    x = MaxPooling2D()(x)        # pool_size=(2, 2), strides=2, padding='same'
...
def get_paper_model(img_height, img_width, num_classes):
    inputs = Input(shape=(img_height, img_width, 3))
